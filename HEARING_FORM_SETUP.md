...
**このテーマは、そのままWordPressにインストールして使用できます。追加の設定は不要です。**
...
```
infinity-lp/
├── functions.php                           # テーマ機能の初期化(フォーム機能を含む)
├── style.css                               # テーマのメタ情報
├── inc/
...
このテーマをWordPressの `wp-content/themes/` ディレクトリにアップロードしてください。
...
または、ZIPファイルとしてダウンロードして、WordPress管理画面からアップロードすることもできます。

### 2. テーマの有効化

WordPress管理画面から、「外観」→「テーマ」で「Infinity Design LP」を有効化してください。
...
- **フロントページ**: LP専用のデザイン
- **ヒアリングフォーム**: ショートコードで表示可能
- **レスポンシブデザイン**: スマートフォンからPCまで対応
- **アニメーション**: 高度なパーティクルアニメーション
